...
export default function PayoffCalculator() {
  ...
  const [payoffResults, setPayoffResults] = useState<any>(null);
  ...
  const calculatePayoff = () => {
    ...
    const loan = loans.find((l: any) => l.id === selectedLoanId);
    if (!loan) return;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center space-x-3">
              ...
              <h1 className="text-xl font-bold text-gray-900">Payoff Calculator</h1>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
                  ...
                  <SelectContent>
                    {loans.map((loan: any) => (
                      <SelectItem key={loan.id} value={loan.id}>
                        {loan.name} - {formatCurrency(parseFloat(loan.outstandingAmount))} @ {loan.interestRate}%
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              {selectedLoanId && loans.find((l: any) => l.id === selectedLoanId) && (
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="space-y-1 text-sm text-blue-800">
                        <p>Outstanding: {formatCurrency(parseFloat(loan.outstandingAmount))}</p>
                        <p>Current EMI: {formatCurrency(parseFloat(loan.emiAmount))}</p>
            ...
            <CardContent>
              {payoffResults ? (
                <div className="space-y-6">
                  {/* Current Scenario */}
                  <div>
                    <h4 className="font-semibold text-gray-900 mb-3">Current Payment Plan</h4>
                    <div className="bg-gray-50 p-4 rounded-lg space-y-2">
                      <div className="flex justify-between">
                        <span className="text-gray-600">Monthly Payment:</span>
                        <span className="font-medium">{formatCurrency(payoffResults.current.monthlyPayment)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Time to Payoff:</span>
                        <span className="font-medium">{payoffResults.current.months} months</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Total Interest:</span>
                        <span className="font-medium">{formatCurrency(payoffResults.current.totalInterest)}</span>
                      </div>
                      <div className="flex justify-between border-t pt-2">
                        <span className="text-gray-600">Total Payment:</span>
                        <span className="font-semibold">{formatCurrency(payoffResults.current.totalPayment)}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Optimized Scenario */}
                  <div>
                    <h4 className="font-semibold text-gray-900 mb-3">With Extra Payment</h4>
                    <div className="bg-green-50 p-4 rounded-lg space-y-2">
                      <div className="flex justify-between">
                        <span className="text-gray-600">Monthly Payment:</span>
                        <span className="font-medium">{formatCurrency(payoffResults.optimized.monthlyPayment)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Time to Payoff:</span>
                        <span className="font-medium">{payoffResults.optimized.months} months</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Total Interest:</span>
                        <span className="font-medium">{formatCurrency(payoffResults.optimized.totalInterest)}</span>
                      </div>
                      <div className="flex justify-between border-t pt-2">
                        <span className="text-gray-600">Total Payment:</span>
                        <span className="font-semibold">{formatCurrency(payoffResults.optimized.totalPayment)}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <h4 className="font-semibold text-blue-900 mb-3">Your Savings</h4>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600">Select a loan and enter extra payment amount to see payoff scenarios.</p>
                </div>
              )}
            </CardContent>
          </Card>
